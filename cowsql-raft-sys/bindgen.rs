...
pub fn generate() {
    let mut raft = PathBuf::from(&env::var("CARGO_MANIFEST_DIR").unwrap());
    raft.push("raft");
    raft.push("include");
    ...
    let bindings = bindgen::Builder::default()
        .header(raft.join("raft.h").to_string_lossy())
        .allowlist_var("^(RAFT|raft)_.*")
        .allowlist_type("^(RAFT|raft)_.*")
        .allowlist_function("^(RAFT|raft)_.*")
